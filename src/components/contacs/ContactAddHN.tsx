...
export interface ContactAddHNProps {
  isShow: boolean;
  record: Contact;
}

export function ContactAddHN(props: ContactAddHNProps) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="w-full">
        <div className="flex flex-wrap -mx-3">
          ...
          <div className="w-full md:w-1/2 px-3 mb-2">
            <label htmlFor="id_number" className="input-label">RTN</label>
            <input
              type="text"
              id="id_number"
              {...register('id_number')}
              onBlur={(e) => setValue('document', e.target.value)}
              placeholder="0101-1990-00000"
              className="input"
            />
          </div>
